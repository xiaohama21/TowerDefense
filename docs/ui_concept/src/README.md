...
| `ui_develop.png` | `ui_develop.html` | 武将养成（技能 / 转职 / 信物 / 特性页签；**属性列 = 百科同款胶囊条**（2×2：左通用 HUD 图徽（透明符号 `icons_hud/`）· 中标签 · 右数值）；基础信息卡末行 = **经验池行**：金色虚线余额胶囊悬停 → 注入公式浮层 /「分配 ▸」/「直接升级」池内不足置灰；v0.17 首版 / v0.19 勘误重渲（浮层口径对齐文档：参与经验 / 难度 ×1.0·×1.6 / 不含落后补正）/ **v0.22**：属性列胶囊化 + 去顶栏「黄巾布」胶囊 + 技能页签去三处虚线说明框） |
...
| `ui_develop_relic.png` | `ui_develop_relic.html` | 武将养成「信物」页签（**v0.22 新增**）：**左右结构** = 左 198px **双槽位**（槽 1 专属信物锁定占位「未开放」/ 槽 2 可选信物未装备态）+ 右**信物库**列表（「可装备」+「装上」/「装备中 · 黄忠」+「装备」）；**列表不写详情，悬停 → 「信物明细」浮层**（类型 / 效果 / 来源 / 归属）；注脚 = 一件只挂 1 名武将 · 转移二次确认 · 未获得件不入列） |
| `ui_develop_relic_equipped.png` | `ui_develop_relic_equipped.html` | 武将养成「信物」页签**已装备态**（**v0.23 新增**）：左槽 2 已装备（金圆图标 + 「可选 · 已装备」+ 效果行「术法伤害 +12%（仅魔法类型）」+ 槽内「卸下」+ 灰注「写入张飞 · 存盘」）+ 右信物库两行均已装备（「装备中 · 本武将（张飞）」→「卸下」/「装备中 · 黄忠」→「装备」）+ 悬停「信物明细」浮层（归属行含「须二次确认转移」）；**选取交互 = 行上按钮（装上 / 装备 / 卸下）**、不做两步式 |
...
| `ui_encyclopedia.png` | `ui_encyclopedia.html` | 百科 · 武将图鉴：左 2 列网格 + 五页签 + 数值模拟器条；属性列 = 2 列**胶囊条**（一条数据一枚胶囊 = 左通用 HUD 图徽 · 中标签 · 右数值），模拟器结果卡同款透明图徽（**v0.20 图徽改透明符号重渲**） |
| `ui_encyclopedia_enemy.png` | `ui_encyclopedia_enemy.html` | 百科 · 敌人图鉴：基础属性 4 枚**胶囊**（含生命占位）+ **击杀奖励 3 枚独立胶囊**（金币 / 经验 / 军功）+ 特殊行为 / 各档难度（内联图徽）/ 出现关卡（**v0.20 图徽改透明符号重渲**，生命 / 军功待出图） |
...
| `ui_battle_hud_v2.png` | `ui_battle_hud_v2.html` | 战斗局内 HUD v2：深墨绿 + 暖金皮肤 / 顶栏图标化 + 出口波次旗帜出怪（取消「开始第 N 波」按钮）/ 底栏 5 卡塔详情（头像 · 伤害 · 攻速 · 升阶 · 回收，费用胶囊同建造位、不显示等阶、取消选中金圈）/ 地图塔去名称 + 阶级角标 / 军需槽局外选带 · 局内金币购买（v0.37.30 定稿；**v0.20 图徽改透明符号 + 去底槽重渲**；旧稿 `ui_battle_hud.*` 保留为历史存档） |
| `ui_squad_confirm.png` | `ui_squad_confirm.html` | 出征·编队「确认出战」二次确认弹窗（640×557 叠层演示，v0.37.0 存档） |
| `ui_supply.png` | `ui_supply.html` | 战斗局内「军需」面板（**v0.16 重渲 + v0.20 图徽改透明符号、去底槽重渲**：购买入带 → 择时使用——带位区 + 已选带件 / 未解锁示意卡列表 + 详情卡购买按钮 + 按钮四态图例条） |
...
- 2026-09-15 图徽改透明符号口径重渲 `ui_encyclopedia.png` / `ui_encyclopedia_enemy.png` / `ui_battle_hud_v2.png` / `ui_supply.png` / `ui_battle_hud.png` / `ui_victory.png` / `ui_wheel.png` 七屏（图标裸符号落位、去底槽底框与 `.well` 描边，v0.37.51 / UI_CONCEPT v0.20 / UI_LAYOUT v0.20.56 / ART_PROMPTS v0.14，MD5 7F3C7F9A5BE589F3DB6FAC621FE44EF1 / 43C388F8308DE5D6547C633DBA76485F / 0AD43A66266BDDDF591D76CE1C17E945 / 3B79309A4A80224C38733B835E306D43 / 5BDF94F021C79EEBC2F24DBA26594E6F / 805D229384F4F6DE40CEC21A3E5AFD0D / B8564F40A6CA645DB52A8621942842DC）同参数重渲入库，与源码同源；2026-09-15 生命 / 军功图徽入库重渲 `ui_encyclopedia_enemy.png`（「生命（基准）」/「军功」两枚虚线占位 → `icons_hud/stat_hp.png` 心形 / `icons_hud/merit.png` 战功勋章，`.ic.ph` 规则删除，v0.37.52 / UI_CONCEPT v0.21 / UI_LAYOUT v0.20.57 / ART_PROMPTS v0.15 / ENCYCLOPEDIA v0.1.18 / ART_ASSETS v0.18，MD5 447129DFA23CCA537ED139F6255A105E）同参数重渲入库，与源码同源。；2026-09-16 养成四屏改版重渲（v0.22 / 程序 0.8.16.x 不变）：`ui_develop.png` / `ui_develop_job.png` / `ui_develop_promo.png`（属性列改百科式胶囊条 `icons_hud/dmg_physical`·`stat_attack_speed`·`stat_range`·`coin` + 去顶栏「黄巾布」/「练兵令」胶囊 + 技能页签去三处虚线说明框 + 死 CSS 清理）与**新增第 21 屏** `ui_develop_relic.png`（信物页签左右结构 + 悬停「信物明细」浮层），MD5 F299632AD896C0C7F9F3FF4ED7BC1FE1 / 153176AB3A857059E414BE1B34E8ECEA / 1D6446DD3CB8306D4E8B6F35CA04654E / C20F1ECD4D90328FCBA4905DA1DA3E3C）同参数重渲入库，与源码同源。
- 2026-09-16 信物页签「已装备态」第 22 屏入库：`ui_develop_relic_equipped.png`（源码 `ui_develop_relic_equipped.html`，自第 21 屏复制改造——槽 2 已装备态 + 信物库两行均已装备 + 悬停「信物明细」浮层；v0.23 / 程序 0.8.16.x 不变，MD5 20D4E3433AF51F40041F8B216BE75713）；同批 **MD5 记录勘误**：养成四屏（`ui_develop` / `ui_develop_job` / `ui_develop_promo` / `ui_develop_relic`）历史记录值与入库 PNG 不符（属改版中间稿），本机重渲 byte 级复现的实测值 = F299632AD896C0C7F9F3FF4ED7BC1FE1 / 153176AB3A857059E414BE1B34E8ECEA / 1D6446DD3CB8306D4E8B6F35CA04654E / C20F1ECD4D90328FCBA4905DA1DA3E3C（上文 2026-09-16 记录行已就地改正）。
...
- `icons_hud/`：**通用 HUD 图徽（透明符号）**（2026-09-11 随百科图鉴概念稿入库）：伤害类型 3（`dmg_physical` / `dmg_magic` / `dmg_true`）、人物属性 7（`stat_attack_speed` / `stat_range` / `stat_armor` / `stat_exp` / `stat_tenacity` / `stat_armor_pen` / `stat_move_speed`）、负面状态 6（`status_slow` / `status_burn` / `status_vulnerable` / `status_stun` / `status_fear` / `status_knockback`）。均为 160×160 **透明底单符号**（符号占内框 88% 居中、四周等距留白）；**2026-09-15 扩至 21 枚**（+ `coin.png` 金币 / `base_hp.png` 基地生命 / `wave_flag.png` 波次旗帜，立绘级原图取自 `icons_battle/`；+ `stat_hp.png` 生命（心形）/ `merit.png` 军功（战功勋章），2026-09-15 出图、白底直出无底板）。**底色口径（2026-09-15 用户校正「用白底是为了好抠图，不是让你直接用白底，图是要融入当前背景的」）**：出图白底 = **抠图原料**，交付 / 界面件 = **透明符号**——界面里由所在面板背景托底，图标不自带底板、不加底框（ART_PROMPTS v0.14 §2.2 / §5.5）。生成脚本 `prep_icons_hud.py`（PIL；通用图徽走「去白底残边 → 剥离深藏青底板 → 归一化 160 透明画布」，生命 / 军功两件走 `SRC2` 通道「去白底 → 归一化」（白底直出、无底板）；重跑与入库件逐字节一致）。风格锚点 = ART_PROMPTS §5.2~5.4（§2.2 简版 UI 图标级）；外部 AI 生成素材，许可以来源为准。**缺口**：军需条目图标 / 塔阶级角标 / 军需处军功余额落位（生命 / 军功图徽 2026-09-15 已出图入库，提示词见 ART_PROMPTS **§5.6**）；漏怪伤害取基地生命图徽 `base_hp.png`。
- `icons_battle/`：**局内 HUD 图标集（立绘级原始件 / 历史件）**（2026-09-07 随 `ui_battle_hud` 入库、2026-09-11 随 `ui_battle_hud_v2` 扩充）：`coin.png` 金币 / `base_hp.png` 基地生命 / `wave_flag.png` 波次旗帜 / `wave_flag_plate.png` 波次旗帜带底板版（历史备用）/ `dmg_physical.png` · `dmg_magic.png` · `dmg_true.png` 伤害类型 / `stat_attack_speed.png` 攻击速度；早期白底裁剪件 `coin_crop.png` / `base_hp_crop.png` 与原始大图 `*_source.jpg` 保留备查（2026-09-15 新增 `stat_hp_source.jpg` 通用资源网格图 / `merit_source.jpg` 战功勋章单张）（**概念稿 2026-09-15 起统一改用 `icons_hud/` 透明符号，不再引用白底件**）。**复现**：`python prep_hud_icons.py`（PIL，自本地 `F:\godotProject\leonardo.ai\通用HUD\*.jpg` 去白底 / 去底板 / 裁边输出金币与旗帜，`dmg_*` / `stat_attack_speed` 自同目录 `png/` 直接拷入）；再跑 `python prep_icons_hud.py`（抠深藏青底板 + 归一化 → 输出 `icons_hud/` 透明符号；生命 / 军功两件走白底直出通道，合计 **21 枚**）。运行时入库目录 = `assets/ui/icons/`（预留，见 ART_ASSETS §3 v0.17）。
