...
| 字段 | 内容 |
|---|---|
| `schema_version` | 存档版本，迁移用（**当前 v5**，0.8.15.0 起：新增 `exp_pool`（经验池余额；旧档迁移补 0、幂等）+ `items.exp_scroll` 残留数量随迁移清理；v4 为 0.8.14.0 碎片字段清理（`characters[id].shards` 删除）+ 信物双槽迁移（旧 `relic` → `relic_exclusive` 占位、新增 `relic_optional`）；v3 含 `squad_character_ids` / `squad_relic_ids` 编队记忆——v0~v4 旧档逐版自动迁移、幂等；0.8.16 落地时按需再升版（军功 + 军需字段）） |
...
- **结算流程（已实现，禁止改动核心语义）**：`BattleSession`（会话内 pending：经验/掉落/解锁/信物/**科技点**/难度标记；✅ 0.8.15.0 起 + 经验池注入（`kill_xp_base` 累计 + 结算 `finalize_exp_pool_injection()`），0.8.16 起 + 军功）→ 胜利 `mark_victory` → `PlayerProfile.apply_battle_session()`（写经验/掉落/解锁/关卡进度/科技点/难度通关记录；**✅ 0.8.15.0 起一并写 `exp_pool`**，0.8.16 起 + `military_merit`）→ `SaveManager.save_profile()`（原子写）；局内遗物**永久使用**（✅ v0.33.1：选带与结算均不消耗库存，v0.22.0"胜利扣库存各 1 件"作废，见 BUGS B-019）。
