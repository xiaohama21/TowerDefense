...
- **升星【✅ v0.13 已实现】**：重复获取武将 → 碎片 → 消耗碎片升星（1~5 星，逐星碎片需求 20/40/80/160）。升星效果 = `damage_growth_per_level` +5%/星（**成长系数加成，不直接加数值**），星级价值随练级兑现，避免"高星低级碾压低星高级"。存档：`characters[id].stars`；养成界面升星入口（显示碎片持有量与需求）。
