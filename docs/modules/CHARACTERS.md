...
- **羁绊【✅ v0.16.0 阶段 5 评估定稿，试点排期阶段 6】**：三国主题的编队组合加成（桃园结义：刘关张同队攻击 +5%、五虎将等）。数据：`BondData`（成员 ids + 效果），编队界面展示；不加数值海，只做构筑彩蛋。**评估结论（v0.16.0）**：①机制面——与现有"出战即生效"管线（升星/信物经 `compute_stats_at`）同构，落地成本低，不动存档结构可只做只读加成（编队时计算）；②数值面——取"同队攻击 +5%"级别的单个小加成即可，避免与科技树全局加成（≤±15%）叠加失控，羁绊加成建议计入 `finalize_damage` 前乘法区；③UI 面——编队界面角色卡显示羁绊标签（如"桃园结义 2/3"），不做独立面板；④风险——九人仅 2~3 组羁绊可组，需防"最强组合唯一解"，建议首批只做桃园结义 + 五虎将预览；⑤排期——阶段 6 与多分支转职试点一并落地（数据 + 编队展示 + 3 个断言）。
