...
| `catapult.tres`【阶段 3 待建】 | 投石车 | DAMAGE | AREA | `lob_aoe` | 1.6 | 1.5 | 2.0（min_range 160） |

说明：
- 职业乘数是相对"角色基础属性"的修正（角色 `.tres` 存绝对值，职业乘数在应用时相乘）；数值基调见 [NUMBERS.md](NUMBERS.md) 10.2/10.3。
- 近战类（骑兵 `single_target_burst`、剑客 `melee_thrust`）均为直伤 + 挥击表现，无弹道；职业克制查询按 profession_id（剑客对 `cavalry` 标签 +15%）。弹道类（弓箭手/术士/舞娘）仍为单体弹道结算，范围/光环随阶段 3 拆分（见 [BEHAVIORS.md](BEHAVIORS.md) B.2/B.3.1）。
- 剑客已落地（v0.9）：攻速偏快由角色 `attack_interval` 承载（张飞 0.7s）。v0.9.2 起刘备亦挂剑客职业（辅助型剑客，`rage_gain_mode=support`，见 4.6/CHARACTERS.md）。
