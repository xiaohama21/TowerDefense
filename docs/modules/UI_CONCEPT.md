...
- **v0.37.0 提交 11 两屏（2026-09-08 概念初稿，待拍板）**：编队「确认出战」二次确认弹窗 `ui_squad_confirm`（v0.33.1 文字规则的可视化定稿——关卡/难度元数据、出战武将名单、遗物清单、取消/确认）与局内「军需」面板 `ui_supply`（局内 HUD 场景变体——军需卡列表 + 右侧详情预览；**悬停 = 即时预览、点击 = 金框固定选中、按「购买」即买即用（无二次确认弹窗）**；弹窗不暂停战斗；列表可纵向扩展、超高滚动兜底；军需图标虚线圆占位待美术）。
